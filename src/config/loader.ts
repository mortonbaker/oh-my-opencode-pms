...
const PROMPTS_DIR_NAME = 'oh-my-opencode-pms';
...
function loadConfigFromPath(
  ...
  try {
    ...
    try {
      ...
      if (!options?.silent) {
        console.warn(
          `[oh-my-opencode-pms] Invalid JSON in ${configPath}:`,
          message,
        );
      }
      return null;
    ...
    if (!result.success) {
      ...
      if (!options?.silent) {
        console.warn(`[oh-my-opencode-pms] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      if (!options?.silent) {
        console.warn(
          `[oh-my-opencode-pms] Error reading config from ${configPath}:`,
          error.message,
        );
      }
    }
...
export function loadPluginConfig(
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      if (!options?.silent) {
        console.warn(`[oh-my-opencode-pms] ${message}`);
      }
    }
  }
  ...
  const readFirstPrompt = (
    ...
    for (const dir of promptSearchDirs) {
      ...
      try {
        ...
        console.warn(
          `[oh-my-opencode-pms] ${errorPrefix} ${promptPath}:`,
